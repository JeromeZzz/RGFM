...
class RelBenchAdapter:
    ...
    def _extract_enhanced_features(self, table_name, df) -> torch.Tensor:
        ...
        if curr_dim > target_dim:
            ...
            pad = torch.zeros((len(df), target_dim - curr_dim))
            return torch.cat([full_feat, pad], dim=1)
    ...
    def _extract_timestamps(self, table_name, df) -> Optional[torch.Tensor]:
        ...
        for col in cols:
            try:
                if pd.__version__ >= '2.0.0': ts = pd.to_datetime(df[col], utc=True, format='mixed', errors='coerce')
                else: ts = pd.to_datetime(df[col], utc=True, errors='coerce')
    ...
    def get_task_config(self, task_name: str) -> TaskConfig:
        ...
        if task:
            ...
            if 'LINK_PREDICTION' in str(tt) or tt == getattr(TaskType, 'LINK_PREDICTION', 'link_prediction'):
                task_type = 'link_prediction'
                num_classes = 2
            elif str(tt) == 'TaskType.BINARY_CLASSIFICATION' or tt == TaskType.BINARY_CLASSIFICATION:
    ...
    def _extract_data(self, table, task) -> Dict:
        ...
        if not etype:
            ...
            else: etype = db_tables[0]
        
        tcol = getattr(task, 'target_col', 'label')
        timecol = getattr(task, 'timestamp_col', None)
        
        # 1. Extract Source Entities
        entity_id_col = None
        for c in df.columns:
            if c != tcol and c != timecol and ('id' in c.lower() or c == etype) and 'item' not in c and 'artist' not in c:
                entity_id_col = c
                break
        ...
        # 2. Extract Destination Entities (For Link Pred)
        dst_entities = None
        potential_dst_cols = [c for c in df.columns if c != entity_id_col and c != tcol and c != timecol and 'id' in c.lower()]
        
        if potential_dst_cols:
            dst_col = potential_dst_cols[0] 
            dst_type = dst_col.replace('_id', '')
            if dst_type not in self.node_id_map:
                 for t in self.node_id_map.keys():
                     if t in dst_col: 
                         dst_type = t
                         break
            
            if dst_type in self.node_id_map:
                dst_map = self.node_id_map[dst_type]
                dst_mapped = df[dst_col].map(dst_map).fillna(0).astype(int).tolist()
                dst_entities = [(dst_type, i) for i in dst_mapped]
        
        # Labels
        if tcol in df.columns: labels = pd.to_numeric(df[tcol], errors='coerce').fillna(0).values
        ...
        if timecol and timecol in df.columns:
            try:
                if pd.__version__ >= '2.0.0': ts = pd.to_datetime(df[timecol], utc=True, format='mixed', errors='coerce')
                else: ts = pd.to_datetime(df[timecol], utc=True, errors='coerce')
                now = pd.Timestamp.now(tz='UTC')
                timestamps = ts.fillna(now).tolist()
            except: pass
                
        return {'entities': entities, 'dst_entities': dst_entities, 'labels': labels, 'timestamps': timestamps}
